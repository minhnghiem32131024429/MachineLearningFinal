...
def analyze_facial_expression_advanced(detections, img_data, depth_map=None, sports_analysis=None):
    ...
    try:
        ...
        # 3. PHÁT HIỆN KHUÔN MẶT VỚI PHƯƠNG PHÁP CẢI TIẾN
        print("Bắt đầu phát hiện khuôn mặt với phương pháp cải tiến...")

        # Sử dụng hàm phát hiện khuôn mặt cải tiến
        faces = detect_faces_improved(subject_img)

        # Nếu không phát hiện được khuôn mặt, thử phát hiện trong phần đầu
        if len(faces) == 0:
            print("Không phát hiện được khuôn mặt, thử phát hiện trong phần đầu...")
            ...
            if head_region.size > 0:
                ...
                # Phát hiện trong vùng đầu
                faces = detect_faces_improved(head_region)

                # Điều chỉnh tọa độ khuôn mặt về khung hình gốc
                if len(faces) > 0:
                    adjusted_faces = []
                    for (x, y, w, h) in faces:
                        adjusted_faces.append((x, y, w, h))
        ...
        if face_found:
            ...
            # Chọn khuôn mặt tốt nhất thay vì chỉ lấy khuôn mặt lớn nhất
            best_face = select_best_face(faces, subject_img)
